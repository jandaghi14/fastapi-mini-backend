...
def get_all_todos(current_user, db, user_id: int = None, limit=10, offset=0, due_date_from: datetime = None):
    ...
    if user_id is not None:
        if user_repository.get_user_by_id_db(user_id, db):
            return todo_repository.get_all_todos_db(db, user_id=user_id,
...
def get_all_todos_with_username(current_user: int, db, user_id: int = None,
                                limit=10, offset=0,
                                due_date_from: datetime = None):
    if user_id is not None:
        if user_repository.get_user_by_id_db(user_id, db):
            return todo_repository.get_all_todos_with_username_db(user_id,
                                                                  db,
                                                                  limit=limit, offset=offset,
                                                                  due_date_from=due_date_from)
        else:
    ...
    if user_repository.get_user_by_id_db(current_user, db):
        return todo_repository.get_all_todos_with_username_db(current_user,
                                                              db,
                                                              limit=limit, offset=offset,
                                                              due_date_from=due_date_from,
