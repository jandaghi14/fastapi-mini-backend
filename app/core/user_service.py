...
def delete_user(user_id, current_user, db):
    ...
    if user is None:
        logger.warning('An unsuccessful try to delete with unavailable username in database')
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")

    if user.role.value == 'admin':
        logger.warning('An unsuccessful try to delete Admin users')
...
def get_user_by_id(user_id: int, db):
    user = user_repository.get_user_by_id_db(user_id, db)
    logger.info("Hitting the database to get_user_by_id")
    if user is None:
